...
function getDespatchList(ioType, dbModel, member, req, res, next, cb){
	...
	if(req.query.partyName){
		if(ioType==0){
			filter['deliveryCustomerParty.party.partyName.name.value']={ '$regex': '' + req.query.partyName + '.*' ,'$options': 'i' }
		}else{
			filter['despatchSupplierParty.party.partyName.name.value']={ '$regex': '' + req.query.partyName + '.*' ,'$options': 'i' }
		}
